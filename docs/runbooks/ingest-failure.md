...
| Database unavailable | Readiness is `NOT_SERVING`; ingest returns `database_unavailable`, retryable, without a receipt | Retry identically after readiness recovers | Restore SurrealDB quorum/connectivity; do not delete archive objects during recovery |
| Trace identity conflict | `trace_conflict`, non-retryable; an orphan canonical archive object may exist | Do not retry with a new idempotency key; issue a new client trace identity for a distinct run | Investigate a client that reused one trace identity for changed events; never overwrite the prior trace |
