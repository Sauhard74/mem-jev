...
Do not replay by rerunning current indexes. For incident reconstruction, load the stored run and captured retrieval documents, validate every content hash, and regenerate the response from the persisted ranks. Validate `request_context_hash` as well as `query_hash`; the former binds the server-inferred tenant, recall permission, and residency allow-list that affected eligibility. A missing historical document, manifest mutation, duplicate document, context mismatch, or hash mismatch is corruption: stop serving that tenant and preserve the records for investigation.
